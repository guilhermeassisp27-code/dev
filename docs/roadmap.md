...
## Já entregue
- **Funil de Vendas — drag and drop, notas, lembretes e Dashboard ampliado**
  _Dor:_ funil Kanban arrastável é apontado como recurso indispensável em CRM imobiliário (previsibilidade de vendas, poucos cliques) e a falta de follow-up estruturado com lembretes é causa direta de leads parados/perdidos; o dashboard anterior (1 gráfico fixo, sem taxa de conversão) estava abaixo do padrão de mercado 2026.
  _Valor:_ cards arrastáveis entre colunas (HTML5 DnD nativo, sem lib), notas/histórico por lead, lembretes com data e destaque visual quando vencem, e Dashboard com taxa de conversão, tempo médio até fechar, ranking dos leads mais valiosos e filtro de período no gráfico de vendas. (2026-06-26)
- **Redesign visual estilo Apple (tool.html) + tema claro/escuro** — repaginação de todas as telas do CRM (sidebar, topbar, Dashboard, Proposta, Leads, Funil, Imóveis, Vendas, Contratos, Tutoriais, modais) com tipografia maior/espaçada, cards com sombra suave e cantos arredondados, botões em pílula, microanimações de 150-250ms e fonte do sistema (sem dependência externa). Novo tema claro (`data-tema="claro"`) ao lado do escuro já existente, com botão de alternância na sidebar que persiste a escolha em `localStorage` e respeita `prefers-color-scheme` no primeiro acesso. _Dor:_ visual datado pesa na percepção de "ferramenta paga" vs. concorrentes (Tecimob, Kenlo) que investem em UI; falta de tema claro era pedido recorrente de quem usa a ferramenta de dia/em ambiente claro. Mudança puramente visual — nenhuma lógica, estrutura de dados ou fluxo de salvamento foi alterada. (2026-06-24)
