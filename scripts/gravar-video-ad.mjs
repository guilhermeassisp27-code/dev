...
await record(browser, 'seg4-proposta', TOOL, async page => {
  ...
  await page.evaluate(() => {
    document.getElementById('f-cliente').value = 'João Pereira'
    document.getElementById('f-tipo-imovel').value = 'Apartamento'
    document.getElementById('f-tipo-neg').value = 'Compra'
    document.getElementById('f-end').value = 'Av. Paulista, 1000 — Bela Vista, SP'
    document.getElementById('f-area').value = '78'
    document.getElementById('f-dorms').value = '3'
    document.getElementById('f-vagas').value = '2'
    document.getElementById('f-valor').value = '450.000'
    if (document.getElementById('f-prazo')) document.getElementById('f-prazo').value = '300'
    if (document.getElementById('f-taxa')) document.getElementById('f-taxa').value = '10.5'
    atualizarTipoNeg()
  })
  await sleep(800)
  ...
  await page.evaluate(async () => {
    ...
    const alvo = top + 520
    await new Promise(res => {
      let y = top; window.scrollTo(0, top)
      const iv = setInterval(() => {
        y += 5; window.scrollTo(0, y)
        if (y >= alvo) { clearInterval(iv); res() }
      }, 16)
